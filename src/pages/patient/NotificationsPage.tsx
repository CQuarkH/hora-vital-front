  ...
  const loadNotifications = async () => {
    ...
    if (response.data && response.success) {
      const mapped = response.data.notifications.map(
        (n: any): UINotification => {
          const typeMap: Record<
            string,
            import("../../types/notification/notification_types").NotificationType
          > = {
          ...
          const priority =
            n.priority &&
            (["Alta", "Media", "Baja"] as const).includes(n.priority)
              ? n.priority
              : "Media";
          ...
          return {
            ...
            type: uiType,
            title:
              n.title || (n.message ? n.message.slice(0, 40) : "Notificación"),
            message: n.message || "",
