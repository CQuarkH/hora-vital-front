...
const STATUS_OPTIONS: (AppointmentStatus | "Todos")[] = [
  "Todos",
  "Confirmada",
  "Pendiente",
  "Completada",
...
export default function AdminAppointmentsPage() {
  ...
  const fetchAppointments = async () => {
    try {
      ...
      const mappedAppointments: AdminAppointment[] = response.appointments.map(
        (apt: any) => ({
          id: apt.id,
  ...
  const filteredAppointments = useMemo(() => {
    return appointments
      ...
      .filter(
        (cita) => doctorFilter === "Todos" || cita.doctorName === doctorFilter
      )
      .filter(
        (cita) => statusFilter === "Todos" || cita.status === statusFilter
      )
      .filter(
        (cita) =>
