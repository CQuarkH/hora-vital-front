    ...
    return (
        <div className="flex items-center gap-4 p-4 bg-medical-50 border border-medical-200 rounded-xl shadow-sm w-full">
            <div className="flex flex-col items-center justify-center p-3 rounded-lg bg-white border border-gray-200">
                ...
                <span className="text-sm font-medium text-medical-700 uppercase">{month}</span>
            </div>

            <div className="flex-1 space-y-1">
                <div className="flex justify-between items-start">
                    <h3 className="font-bold text-medical-900">{appointment.doctorName}</h3>
                    <AppointmentStatusBadge status={appointment.status} />
                </div>
                <p className="text-sm text-gray-600">{appointment.specialty}</p>
                <div className="flex items-center gap-4 text-sm text-gray-500 pt-1">
                    <span className="flex items-center gap-1"><HiOutlineCalendar /> {appointment.date}</span>
                    <span className="flex items-center gap-1"><HiOutlineClock /> {appointment.time}</span>
                    <span className="flex items-center gap-1"><HiOutlineLocationMarker /> {appointment.location}</span>
                </div>
            </div>
